...
// ✅ CORRECTED: All 65 students matched exactly to official PDF (PIN → Name)
...
const DEFAULT_PAYMENTS = {
  ...
  '25007-CS-009': { paid: false, amount: '',    date: '',           time: '',      method: 'UPI'  },
  ...
  '25007-CS-019': { paid: false, amount: '',    date: '',           time: '',      method: 'UPI'  },
  ...
  '25007-CS-022': { paid: false, amount: '',    date: '',           time: '',      method: 'UPI'  },
  ...
  '25007-CS-027': { paid: false, amount: '',    date: '',           time: '',      method: 'UPI'  },
  ...
  '25007-CS-030': { paid: false, amount: '',    date: '',           time: '',      method: 'UPI'  },
  ...
  '25007-CS-033': { paid: false, amount: '',    date: '',           time: '',      method: 'UPI'  },
...
try {
  if (fs.existsSync(DB_PATH)) {
    paymentMap = JSON.parse(fs.readFileSync(DB_PATH, 'utf8'));
    console.log('✅ Loaded saved data from', DB_PATH);
  } else {
    paymentMap = { ...DEFAULT_PAYMENTS };
    fs.writeFileSync(DB_PATH, JSON.stringify(paymentMap, null, 2), 'utf8');
    console.log('✅ Seeded default payment data to', DB_PATH);
  }
} catch (e) {
  console.error('⚠️  Data load error, using defaults:', e.message);
  paymentMap = { ...DEFAULT_PAYMENTS };
}
...
app.get('/api/stats', (req, res) => {
  ...
  res.json({
    total: s.length,
    paid: paid.length,
    unpaid: s.length - paid.length,
    totalAmount: paid.reduce((a, x) => a + (parseFloat(x.amount) || 0), 0)
  });
});
